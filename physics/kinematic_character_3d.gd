...
## when landing from air onto a sloped floor. Additionally, [member velocity] will keep it's
## horizontal direction when solving a floor constraint. (The horizontal plane is defined by
...
## Sets a maximum snapping distance. When set to a value different from [code]0.0[/code], the body
## is kept attached to slopes when calling [method move_and_slide]. The snapping vector is
## determined by the given distance along the opposite direction of the [member up_direction].
## If the body snaps to the floor, [member velocity] will be modified so that it parallels the floor
## plane.
...
## If [code]true[/code], the applied motion from [member platform_velocity] will also follow the
## sliding behaviour, instead of stopping on collisions.
...
## accurate, when you leave the last platform velocity is applied. See [enum PlatformLeaveAction]
...
## platforms to be followed by the body.
...
## If [code]false[/code], rigid bodies collisions will be treated as collisions with static bodies.
## [br] If [code]true[/code], contact impulses and weight force will be simulated when colliding
## with rigid bodies. In this case, it's [b]important[/b] that the collision mask of the rigid
## bodies that will be interacted with has the character's collision layer excluded!
...
## [br] Set during [method move_and_slide].
var prev_on_floor: bool
## [code]true[/code] if the body is touched a floor surface in the previous call to
## [method move_and_slide]. [br] Set during [method move_and_slide].
...
## The normal of the deepest active floor collision. If the body is not touching a floor, it's a
## zero vector. [br] Set during [method move_and_slide].
var current_floor_normal: Vector3
## The normal of the deepest active wall collision. If the body is not touching a wall, it's a zero
## vector. [br] Set during [method move_and_slide].
var current_wall_normal: Vector3
## The normal of the deepest active ceiling collision. If the body is not touching a ceiling, it's a
## zero vector. [br] Set during [method move_and_slide].
var current_ceiling_normal: Vector3
## The normal of the latest floor collision. If the body did not collide with a floor, it's a zero
## vector. [br] Set during [method move_and_slide].
var last_floor_normal: Vector3
## The normal of the latest wall collision. If the body did not collide with a wall, it's a zero
## vector. [br] Set during [method move_and_slide].
var last_wall_normal: Vector3
## The normal of the latest ceiling collision. If the body did not collide with a ceiling, it's a
## zero vector. [br] Set during [method move_and_slide].
...
## The velocity the body hit the ground with arriving from air. If that didn't happen, it's a zero
## vector. [br] Set during [method move_and_slide].
...
## [br] Set during [method move_and_slide].
var _prev_platform_velocity_valid: bool
...
static func is_body_dynamic(rid: RID) -> bool:
	...
	return (
		body_mode == PhysicsServer3D.BodyMode.BODY_MODE_RIGID
		or body_mode == PhysicsServer3D.BodyMode.BODY_MODE_RIGID_LINEAR
	)
...
static func _append_kinematic_constraint_if_unique_approx(
	...
	for other_constraint: KinematicConstraint in p_constraints:
		...
		for i: int in 3:
			var c1: float = p_constraint.plane_normal[i]
			var c2: float = other_constraint.plane_normal[i]
			if c1 > c2 - PLANES_CMP_EPSILON and c1 < c2 + PLANES_CMP_EPSILON:
				equal_component_count += 1
			else:
				break
		if (
				equal_component_count == 3
				and other_constraint.velocity.is_equal_approx(p_constraint.velocity)
		):
			return false
...
static func solve_kinematic_constraints(
	...
	var new_velocity: Vector3 = p_velocity
	...
	for i: int in MAX_CONSTRAINT_ITERATIONS:
		...
		var constraint: KinematicConstraint = get_constraint_with_highest_penetration(
				p_constraints, new_velocity
		)
		...
		if other_constraint:
			# Jolt: "Cancel the constraint velocity in the other constraint plane's direction so
			# that we won't try to apply it again and keep ping ponging between planes"
			# ... but only if the constraint velocity is pushing.
			if constraint.velocity.dot(constraint.plane_normal) >= 0.0:
				constraint.velocity -= (
						minf(0.0, constraint.velocity.dot(other_constraint.plane_normal))
						* other_constraint.plane_normal
				)
			if other_constraint.velocity.dot(other_constraint.plane_normal) >= 0.0:
				other_constraint.velocity -= (
						minf(0.0, other_constraint.velocity.dot(constraint.plane_normal))
						* constraint.plane_normal
				)
			
			# If applying one constaint velocity solves the other's,
			# don't add them together to avoid duplicate push velocity
			var combined_constraint_velocity: Vector3
			var c_vel_diff: Vector3 = constraint.velocity - other_constraint.velocity
			if (c_vel_diff).dot(other_constraint.plane_normal) >= -CMP_EPSILON:
				combined_constraint_velocity = constraint.velocity
			elif (-c_vel_diff).dot(constraint.plane_normal) >= -CMP_EPSILON:
			...
			var slide_dir: Vector3 = (
					constraint.plane_normal.cross(other_constraint.plane_normal).normalized()
			)
			var relative_velocity: Vector3 = new_velocity - combined_constraint_velocity
			var rv_in_slide_dir: Vector3 = relative_velocity.dot(slide_dir) * slide_dir
			...
			if other_constraint.constant_speed:
				if constraint.constant_speed:
					# If both constrains are marked, modify the projected relative velocity to have
					# the original's length
					rv_in_slide_dir = rv_in_slide_dir.normalized() * relative_velocity.length()
				else:
					# If only the other constraint is marked, make relative velocity parallel to the
					# marked constraint without changeing its length, than project that onto the
					# crease direction.
					var oc_slide_dir: Vector3 = (
							relative_velocity.slide(other_constraint.plane_normal).normalized()
					)
					var oc_slide_vel: Vector3 = oc_slide_dir * relative_velocity.length()
					
					rv_in_slide_dir = oc_slide_vel.dot(slide_dir) * slide_dir
		...
		if not constraint.velocity.is_zero_approx():
			...
			return Vector3.ZERO # No more degress of freedom left, so stop dead.
...
static func solve_constraint(
	...
	if is_angle_too_perpendicular(
			p_constraint.plane_normal, p_constraint.min_slide_angle, relative_velocity
	):
	...
	if p_constraint.constant_speed or p_constraint.keep_horizontal_dir:
		...
		if p_constraint.keep_horizontal_dir:
			...
			slide_dir = (
					...
					.cross(p_constraint.plane_normal)
					.normalized()
			)
	...
	var new_velocity: Vector3 = (
			p_velocity
			- relative_velocity
			.dot(p_constraint.plane_normal)
			* p_constraint.plane_normal
	)
...
static func solve_dynamic_constraints(
	...
	for constraint: DynamicConstraint in p_constraints:
		...
		var j_denom: float = (
				(m1_inv) + (m2_inv) + (normal.dot((r2.cross(normal) * I2_inv).cross(r2)))
		)
		...
		if (
				...
				and not BodySaveState.array_contains_rid(
						p_io.body_save_states, constraint.collider_rid
				)
		):
...
func move_and_slide() -> bool:
	...
	has_stepped = false
	# We will read this later in grounded move, so don't clear it yet
	...
	var pre_move_constraints: Array[KinematicConstraint] = _handle_pre_move_contacts()
	
	# Move with requested motion mode
	if motion_mode == MotionMode.GROUNDED:
		_grounded_move(delta_t, pre_move_constraints)
	else:
		last_floor_normal = Vector3.ZERO
		_floating_move(delta_t, pre_move_constraints)
	
	# Add platform velocity if left floor
	if (
			platform_leave_action != PlatformLeaveAction.DO_NOTHING
			and not platform_velocity.is_zero_approx() and not current_floor_collider_encoded
	):
...
func snap_to_floor(p_snap_length: float) -> bool:
	...
	if not (result_state.s_floor or result_state.s_wall_floor):
		...
		if _collided_with_floor_surface(result_state, snap_position):
	...
	for i: int in result_state.floor_collision_indexes:
		...
		_append_kinematic_constraint_if_unique_approx(
		...
		if d < floor_detach_threshold:
			_append_kinematic_constraint_if_unique_approx(
					...
					KinematicConstraint.new(
							- result_state.wall_floor_normal, Vector3.ZERO, 0.0, true, true
					)
			)
	...
	if not detaching_from_floor or (detaching_from_floor and not is_travel_significant):
		_update_overall_state(result_state, true, false, false, true)
...
func _handle_pre_move_contacts() -> Array[KinematicConstraint]:
	...
	# is suitable for collision checks, since it's the only way to retrieve
	# the necessary data of all contacts.
	...
	_set_collision_state(pre_move_motion_result, incoming_collision_state, true)
	...
	if motion_mode == MotionMode.FLOATING:
		_update_overall_state(incoming_collision_state, false, true, false)
	else:
		_update_overall_state(incoming_collision_state, true, true, true)
	
	# Determine constraints from this state
	var kinematic_constraints: Array[KinematicConstraint]
	var dynamic_constraints: Array[DynamicConstraint]
	if motion_mode == MotionMode.FLOATING:
		_determine_floating_move_constraints(
			incoming_collision_state, kinematic_constraints, dynamic_constraints
		)
	else:
		_determine_grounded_move_constraints(
			incoming_collision_state, kinematic_constraints, dynamic_constraints
		)
	...
	_set_collision_state(pre_move_motion_result, pre_move_collision_state, false)
...
func _handle_contacts_at_final_position() -> void:
	# TODO: A better method is needed for checking shape intersections. Replace
	# this collision check for that one, once available.
	...
	_set_collision_state(touch_motion_result, touch_collision_state)
	
	# Update overall state and also set current state properties
	if motion_mode == MotionMode.FLOATING:
		_update_overall_state(touch_collision_state, false, true, false, true)
	else: # Grounded move
		...
		_update_overall_state(touch_collision_state, false, true, true, true)


## Performs a grounded movement.
func _grounded_move(p_delta_t: float, p_pre_move_constraints: Array[KinematicConstraint]) -> void:
	...
	if current_floor_collider_rid.is_valid():
		var platform_layer: int = PhysicsServer3D.body_get_collision_layer(
				current_floor_collider_rid
		)
		...
		if not excluded:
			...
			if _prev_platform_velocity_valid:
				var platform_vertical_velocity_change_length: float = (
						prev_platform_velocity - platform_velocity).dot(last_floor_normal
				)
				if (
						platform_vertical_velocity_change_length > platform_detach_treshold
						and prev_platform_velocity.dot(last_floor_normal) > floor_detach_threshold
				):
	...
	_prev_platform_velocity_valid = floor_collider_valid
	...
	if not platform_velocity.is_zero_approx():
		if slide_platform_motion:
			...
			var platform_move_io := CollideAndSlideIO.new(
					global_transform, platform_velocity, p_delta_t
			)
			_collide_and_slide(
					platform_move_io,
					prev_on_floor,
					p_pre_move_constraints,
					[current_floor_collider_rid]
			)
			...
			for collision_state: CollisionState in platform_move_io.collision_states:
				...
				_update_overall_state(collision_state, true, true, true)
		else:
			...
			if collided:
				var result_state := CollisionState.new()
				_set_collision_state(motion_result, result_state)
				_update_overall_state(result_state, true, true, true)
	...
	# Move with velocity
	var move_io := CollideAndSlideIO.new(global_transform, velocity, p_delta_t)
	_collide_and_slide(move_io, prev_on_floor, p_pre_move_constraints)
	...
	if interact_with_rigid_bodies:
		_apply_gravity_force_to_floor(p_delta_t)


## Applies gravity force to the floor collider.
func _apply_gravity_force_to_floor(p_delta_t: float) -> void:
	if not (current_floor_collider_encoded and is_body_dynamic(current_floor_collider_rid)):
		return
	
	# Create dynamic constraints
	var index: int = ColliderData.find_by_collider_id(
			collider_datas, current_floor_collider_encoded.object_id
	)
...
func _floating_move(p_delta_t: float, p_pre_move_constraints: Array[KinematicConstraint]) -> void:
	# Clear floor collider data
	current_floor_collider_encoded = null
	current_floor_collider_rid = RID()
	...
	for collision_state: CollisionState in move_io.collision_states:
		collision_states.push_back(collision_state)
		_update_overall_state(collision_state, false, true, false)


## Simulates body movement using collision checks.
func _collide_and_slide(
		p_io: CollideAndSlideIO,
	...
	if motion_mode == MotionMode.FLOATING:
		_determine_floating_move_constraints(
				result_state, kinematic_constraints, dynamic_constraints
		)
		...
		# If no floor collision is reported, we need an extra collision check at our current
		# position.
		if not touching_floor:
			...
			if _check_floor_status(p_io.transform, max_step_height, floor_state, true):
				...
				touching_floor = (
					floor_below
					and floor_check_travel.length_squared() <= (safe_margin * RECOVERY_FACTOR) ** 2
				)
		...
		if (
				p_step_move_enabled
				and not is_zero_approx(max_step_height)
				and (floor_below or floor_surface_below)
				and result_state.s_wall
		):
			# Retrieve IO objects of forward move, in case step move fails
			var step_forward_ios: Array[CollideAndSlideIO]
			if _step_move(p_io, result_state, p_excluded_bodies, step_forward_ios):
			...
			for move_shape_io: CollideAndSlideIO in step_forward_ios:
				move_shape_io.restore_body_states()
		
		# Determine if we want to block horizontal movement on steep slopes
		var slide_up_steep_slopes: bool = (
				not (floor_block_on_wall and touching_floor and result_state.s_wall)
		)
	...
	if floor_stop_on_slope and landed_from_air:
		...
		if p_io.velocity.dot(up_direction) < 0.0:
			...
			new_velocity = solve_kinematic_constraints(
					floor_dir_constraints, new_velocity, up_direction
			)
...
func _determine_floating_move_constraints(
	...
	for i: int in p_collision_state.wall_collision_indexes:
		...
		if interact_with_rigid_bodies and is_body_dynamic(collider_rid):
			...
			p_dynamic_constraints.push_back(
					DynamicConstraint.new(collider_rid, wall_normal, collision_point)
			)
			continue
		...
		_append_kinematic_constraint_if_unique_approx(
				...
				KinematicConstraint.new(
						wall_normal, wall_velocity, wall_min_slide_angle, wall_constant_speed
				)
		)


## Determines constraints for grounded movement according to input collision state.
func _determine_grounded_move_constraints(
		p_collision_state: CollisionState,
		p_kinematic_constraints: Array[KinematicConstraint],
		p_dynamic_constraints: Array[DynamicConstraint] = [],
	...
	for i: int in p_collision_state.floor_collision_indexes:
		...
		_append_kinematic_constraint_if_unique_approx(
				p_kinematic_constraints,
				KinematicConstraint.new(
						floor_normal, Vector3.ZERO, 0.0, floor_constant_speed, floor_stop_on_slope
				)
		)
	...
	for i: int in p_collision_state.wall_collision_indexes:
		...
		if interact_with_rigid_bodies and is_body_dynamic(collider_rid):
			var collision_point: Vector3 = p_collision_state.motion_result.get_collision_point(i)
			p_dynamic_constraints.push_back(DynamicConstraint.new(
					collider_rid, wall_normal, collision_point)
			)
			continue
		
		# Convert to constraint
		var wall_constraint := KinematicConstraint.new(
				wall_normal, wall_velocity, wall_min_slide_angle, wall_constant_speed
		)
		...
		if not p_slide_up_steep_slopes:
			...
			if dot > CMP_EPSILON:
				...
				# Jolt: "Project the contact velocity on the new normal so that
				# both planes push at an equal rate"
				var projected_velocity: Vector3 = (
						wall_velocity.dot(horizontal_normal) * horizontal_normal
				)
		...
		_append_kinematic_constraint_if_unique_approx(p_kinematic_constraints, wall_constraint)
	...
	for i: int in p_collision_state.ceiling_collision_indexes:
		...
		if interact_with_rigid_bodies and is_body_dynamic(collider_rid):
			var collision_point: Vector3 = p_collision_state.motion_result.get_collision_point(i)
			p_dynamic_constraints.push_back(DynamicConstraint.new(
					collider_rid, ceiling_normal, collision_point)
			)
			continue
		...
		if not p_slide_up_ceilings:
			...
			_append_kinematic_constraint_if_unique_approx(
						p_kinematic_constraints,
						KinematicConstraint.new(
								down_normal, projected_velocity, 0.0, ceiling_constant_speed
						)
				)
		...
		_append_kinematic_constraint_if_unique_approx(
				p_kinematic_constraints,
				KinematicConstraint.new(
						ceiling_normal, ceiling_velocity, 0.0, ceiling_constant_speed
				)
		)


## Checks if the body is at least the given distance away downward from a floor collision or 
## surface and sets the given CollisionState object.
func _check_floor_status(
	...
	motion_params.recovery_as_collision = true
	motion_params.max_collisions = MAX_SNAP_COLLISIONS
	# We are only testing in this function, but we might apply the travel outside,
	# so enable slide cancelling
	var collided: bool = _move_and_collide(motion_result, motion_params, true, true)
	
	if not collided:
		return false
	
	_set_collision_state(motion_result, p_result_state)
	
	if p_result_state.s_floor or p_result_state.s_wall_floor:
		return true
	
	if (
			p_check_for_surface_normal
			and p_result_state.s_wall
			and _collided_with_floor_surface(p_result_state, p_from.origin)
	):
...
func _step_move(
		...
		p_collision_state: CollisionState,
		p_excluded_bodies: Array[RID] = [],
		step_forward_ios: Array[CollideAndSlideIO] = []
) -> bool:
	...
	for i: int in p_collision_state.wall_collision_indexes:
		...
		if (horizontal_velocity - wall_velocity).dot(wall_normal) < 0.0:
			...
			var motion_length_towards_wall: float = (
					- (base_forward_motion + extra_forward_motion).dot(horizontal_wall_normal)
			)
			extra_forward_motion += (
					maxf(0.0, min_forward_length - motion_length_towards_wall)
	...
	if not extra_forward_motion.is_zero_approx():
		...
		var extra_forward_move_io := CollideAndSlideIO.new(
				step_move_transform, extra_forward_motion, 1.0
		)
		_collide_and_slide(extra_forward_move_io, false, [], p_excluded_bodies, false)
	...
	var forward_move_io := CollideAndSlideIO.new(
			step_move_transform, horizontal_velocity, p_io.time_remaining
	)
	if not is_zero_approx(p_io.time_remaining):
		_collide_and_slide(forward_move_io, false, [], p_excluded_bodies, false)
		# Update step move position
	...
	_set_collision_state(down_motion_result, down_result_state)
	...
	if not down_result_state.s_floor:
		...
		var wall_surface_normal: Vector3 = _get_wall_surface_normal_below(
				down_result_state, horizontal_velocity
		)
		...
		var floor_check_forward_motion: Vector3 = (
				floor_check_forward_dir * step_floor_check_distance
		)
		
		# Move forward with this motion
		var floor_check_forward_io := CollideAndSlideIO.new(
				offsetted_up_transform, floor_check_forward_motion, 1.0
		)
		_collide_and_slide(floor_check_forward_io, false, [], p_excluded_bodies, false)
		...
		if _move_and_collide(floor_check_down_result, down_motion_params, true):
			...
			#var debug_position := (
					#down_motion_result.get_meta(
							#"travel", down_motion_result.get_travel()
					#) as Vector3
					#+ floor_check_forward_io.transform.origin
			#)
			#DebugDraw3D.draw_sphere(debug_position,0.2)
			var floor_check_result_state := CollisionState.new()
			_set_collision_state(floor_check_down_result, floor_check_result_state)
	...
	p_io.velocity = (
		forward_move_io.velocity + maxf(0.0, p_io.velocity.dot(up_direction)) * up_direction
	)
...
func _update_overall_state(
	...
	if p_set_floor and p_col_state.s_floor:
		...
		if p_set_final_state:
			...
			current_floor_collider_encoded.object_id = p_col_state.motion_result.get_collider_id(
					p_col_state.deepest_floor_index
			)
			current_floor_collider_rid = p_col_state.motion_result.get_collider_rid(
					p_col_state.deepest_floor_index
			)
	...
	for collider_data: ColliderData in p_col_state.collider_datas:
		var array_index: int = ColliderData.find_by_collider_id(
				collider_datas, collider_data.collider_id
		)
		if array_index == -1:
...
func _set_collision_state(
	...
	for i: int in collision_count:
		...
		if p_ignore_character_motion:
			...
			if (
					velocity.dot(collision_normal) < -CMP_EPSILON
					or relative_velocity.dot(collision_normal) > 0.0
			):
		...
		var array_index: int = ColliderData.find_by_collider_id(
				p_col_state.collider_datas, collider_id
		)
		if array_index == -1:
		...
		if motion_mode == MotionMode.GROUNDED:
			...
			if _is_ceiling(collision_normal):
		...
		if (
				...
				and not collision_normal.is_equal_approx(tmp_wall_normal)
		):
	...
	if motion_mode == MotionMode.GROUNDED and wall_collision_count > 1:
		...
		if not p_col_state.s_floor:
			...
			if _is_floor(combined_wall_normal):
		...
		if not p_col_state.s_ceiling:
			...
			if _is_ceiling(combined_wall_normal):
...
func _is_floor(p_normal: Vector3) -> bool:
...
func _is_ceiling(p_normal: Vector3) -> bool:
...
func _is_wall(p_normal: Vector3) -> bool:
	return not (_is_floor(p_normal) or _is_ceiling(p_normal))


## Returns the centre-bottom (feet) position of the collider at a given position.
func _get_feet_position(p_global_position: Vector3 = global_position) -> Vector3:
...
## Returns the radius of the collider, if the collider bottom is round, else it returns 0.0.
func _get_collider_bottom_radius() -> float:
	if collider.shape is CapsuleShape3D:
		var capsule := collider.shape as CapsuleShape3D
...
func _collided_with_floor_surface(
		p_collision_state: CollisionState,
		p_global_position: Vector3
) -> bool:
	var raycast := RayCast.new()
	var direct_space_state: PhysicsDirectSpaceState3D = get_world_3d().direct_space_state
	var max_surface_height: float = (
		_get_feet_position(p_global_position).dot(up_direction) + _get_collider_bottom_radius()
	)
	...
	for i: int in p_collision_state.wall_collision_indexes:
		...
		if raycast.hit and _is_floor(raycast.normal):
			return true
...
func _get_wall_surface_normal_below(
		p_collision_sate: CollisionState,
		p_velocity: Vector3
) -> Vector3:
	var raycast := RayCast.new()
	var direct_space_state: PhysicsDirectSpaceState3D = get_world_3d().direct_space_state
	...
	for i: int in p_collision_sate.wall_collision_indexes:
		...
		if raycast.hit and _is_wall(raycast.normal):
...
class CollideAndSlideIO extends RefCounted:
	# Input and output
