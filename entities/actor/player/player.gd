...
func _process(_delta: float) -> void:
	...
	if SETTINGS.player_draw_collision_shape_enabled:
		var _s := DebugDraw3D.new_scoped_config().set_thickness(0.005)
		if player_body.collider.shape is CylinderShape3D:
			var feet_pos: Vector3 = player_body._get_feet_position()
			DebugDraw3D.draw_cylinder_ab(
					feet_pos,
					feet_pos + player_body.collider.global_transform.basis.y * cylinder_shape.height,
					cylinder_shape.radius,
					Color.GREEN
			)
		if player_body.collider.shape is BoxShape3D:
			DebugDraw3D.draw_box(
					player_body.collider.global_position,
					player_body.collider.quaternion,
					box_shape.size,
					Color.GREEN,
					true
		)
		if player_body.collider.shape is CapsuleShape3D:
			var feet_pos: Vector3 = player_body._get_feet_position()
			var up_axis: Vector3 = player_body.collider.global_transform.basis.y
			DebugDraw3D.draw_sphere(
					feet_pos + up_axis * capsule_shape.radius,
					capsule_shape.radius,
					Color.GREEN
			)
			var cylinder_ab: Vector3 = (capsule_shape.height - 2 * capsule_shape.radius) * up_axis
			DebugDraw3D.draw_cylinder_ab(
					feet_pos + up_axis * capsule_shape.radius,
					feet_pos + up_axis * capsule_shape.radius + cylinder_ab,
					cylinder_shape.radius,
					Color.GREEN
			)
			DebugDraw3D.draw_sphere(
...
func _physics_process(_delta: float) -> void:
	...
	if (
			...
			and player_body.is_on_floor
			and (
					player_body.velocity.length_squared() > 0.5 ** 2
					or (player_body.is_on_floor and not player_body.prev_on_floor))
			and not audio_stream_player.playing
	):
		audio_stream_player.stream = footstep_sounds.pick_random()
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if SETTINGS.player_swap_collision_shape_enabled :
